...
public class SelectActivity
    ...
    private boolean chooseContactForRingtone(MenuItem item) {
        try {
            ...
            intent.setClassName(
                    "com.Looper",
                    "com.Looper.ChooseContactActivity");
            startActivityForResult(intent, REQUEST_CODE_CHOOSE_CONTACT);
        } catch (Exception e) {
            Log.e("Looper", "Couldn't open Choose Contact window");
        }
        return true;
    }

    private void confirmDelete() {
        // See if the selected list item was created by Looper to
        // determine which alert message to show
        Cursor c = mAdapter.getCursor();
        String artist = c.getString(c.getColumnIndexOrThrow(
                MediaStore.Audio.Media.ARTIST));
        CharSequence LooperArtist =
                getResources().getText(R.string.artist_name);

        CharSequence message;
        if (artist.equals(LooperArtist)) {
            message = getResources().getText(
                    R.string.confirm_delete_Looper);
        } else {
            message = getResources().getText(
                    R.string.confirm_delete_non_Looper);
        }
    ...
    private void onRecord() {
        try {
            ...
            intent.setClassName("com.Looper", "com.Looper.LooperEditActivity");
            startActivityForResult(intent, REQUEST_CODE_EDIT);
        } catch (Exception e) {
            Log.e("Looper", "Couldn't start editor");
        }
    }
    ...
    private void startLooperEditor() {
        Cursor c = mAdapter.getCursor();
        int dataIndex = c.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA);
        int titleIndex = c.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE);
        try {
            Intent in = new Intent(this,
                    MainActivity.class);
            in.putExtra("songPath", c.getString(dataIndex));
            in.putExtra("songTitle", c.getString(titleIndex));
            setResult(100, in);
            finish();
        } catch (Exception e) {
            Log.e("Looper", "Couldn't start editor");
